...
Nl = [64,100,10]; % number of nodes per layer
regu = logspace(-8,0,10); % regularization factor
stepsize = logspace(-3,-1,10);
...
imn = [55,55,55,55,55,55,55,55,55,55];
samplesize = 100;
...
for l = 1:length(stepsize)
    for k = 1:length(regu)
        [W_old,theta_old] = network_initial(L,Nl); % intitialization of W, theta with random values
        for j = 1:10
            rand_ind = randperm(size(Gamma_train,2));
            for i = 1:size(Gamma_train,2)
                gamma = Gamma_train(:,rand_ind(i));
                h = H_train(:,rand_ind(i));
                [Z,Y] = feedforward(W_old,theta_old,h,L,Nl);
                [W_new,theta_new] = grad_back_ce(Z,Y,L,Nl,regu(k),stepsize(l),gamma,W_old,theta_old);
            ...
            for i = 1:size(Gamma_test,2)
                gamma = Gamma_test(:,i);
                h = H_test(:,i);
                [Z,Y] = feedforward(W_old,theta_old,h,L,Nl); % calculate Z, Y matrices
...
c = sprintf('Max performance = %0.2f',max(max(final_performance/100)))
title(c);
az = 0;
el = 90;
view(az, el);
